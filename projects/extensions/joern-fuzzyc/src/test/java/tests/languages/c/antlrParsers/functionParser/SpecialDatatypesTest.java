...
public class SpecialDatatypesTest extends FunctionParserTestBase {
	...
	public void testStructWithPoint() {
		...
		ParseTree tree = functionParser.parseString(input);
		String output = tree.toStringTree(functionParser.getAntlrParser());
		String expected = "(statements (statement (simple_decl (var_decl (special_datatype struct (identifier stat) (identifier s) = { . st_dev = 0 } ;)))))";
		assertEquals(expected, output);
	}
