...
public class CSVCFGExporter extends CFGExporter {
	...
	protected void writeCFGNode(CFGNode statement, Map<String, Object> properties) {
		properties.put(NodeKeys.FUNCTION_ID, String.format("%d", Writer.getIdForObject(currentFunction)));
	...
	public void writeCFGEdges(CFG cfg) {
		...
		for (CFGEdge cfgEdge : cfg.getEdges()) {
			...
			if ((src instanceof ASTNodeContainer || src instanceof CFGEntryNode || src instanceof CFGExitNode)
				...
				// CFG nodes that are AST node containers have their ids stored in their AST
				// node;
